...
/** `lang` fuerza el idioma (lo usa la landing en inglés /franchise). */
export function FranchiseForm({ lang }: { lang?: 'es' | 'en' } = {}) {
  ...
  /** Arma los datos del lead una sola vez, para correo y para WhatsApp. */
  const buildLead = () => {
  ...
  /**
   * Envío por WhatsApp: abre el chat con toda la solicitud ya redactada.
   * Es la vía directa al teléfono del cliente, además del correo.
   */
  const handleWhatsApp = () => {
    if (!validate()) return
    const { fields, subject } = buildLead()
    const body = [subject, '', ...Object.entries(fields).map(([k, v]) => `${k}: ${v}`)].join('\n')
    window.open(`${SITE.whatsappHref}?text=${encodeURIComponent(body)}`, '_blank', 'noopener,noreferrer')
  }
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-2.5 bg-[#006847] hover:bg-[#00543a] disabled:opacity-70 disabled:cursor-not-allowed text-white font-black text-sm px-8 py-4 rounded shadow-lg transition-all duration-200 uppercase tracking-wider"
              >
                {isSubmitting ? (
              ...
              {/* Vía directa al WhatsApp del cliente, con la solicitud completa. */}
              <button
                type="button"
                onClick={handleWhatsApp}
                className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1ebe5d] text-white font-black text-sm px-6 py-4 rounded shadow-lg transition-all duration-200 uppercase tracking-wider"
              >
                ...
                <span>{t('Enviar por WhatsApp', 'Send via WhatsApp')}</span>
              </button>
            </div>
            {submitError && (
