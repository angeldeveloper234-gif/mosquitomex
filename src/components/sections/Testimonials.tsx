        ...
        <FadeUp>
          <div className="text-center space-y-[1rem] mb-[3rem]">
            <h2 className="text-h2 font-black tracking-tighter uppercase leading-[1.15] text-[#082135] dark:text-slate-100">
              {t('testimonials.titlePre')}<span className="text-[#e82536]">{t('testimonials.titleHighlight')}</span>
            </h2>
            <div className="flex justify-center gap-[0.25rem]">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="fill-[#e82536] text-[#e82536] size-[1.25rem]" />
              ))}
            </div>
          </div>
        </FadeUp>
          ...
          {reviews.map((review, i) => (
            <FadeUp key={i} delay={i * 0.1}>
              <div className="relative p-[2rem] border border-slate-200/60 dark:border-slate-800/60 bg-slate-50/40 dark:bg-slate-900/20 backdrop-blur-xs">
                <Quote className="absolute top-[1.5rem] right-[2rem] text-[#e82536]/10 size-[3rem]" />
                <div className="space-y-[1rem] relative z-10">
                  <p className="text-[#082135] dark:text-slate-300 italic leading-[1.6] font-medium text-[1rem]">
                    "{review.text}"
                  </p>
                  <div className="pt-[1rem] border-t border-slate-200/60 dark:border-slate-800/60">
                    <p className="font-black text-[#082135] dark:text-slate-200 uppercase text-[1rem]">{review.name}</p>
